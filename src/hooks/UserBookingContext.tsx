...
export const UserBookingContext = createContext<UserBookingContextType | undefined>(undefined);

export const UserBookingContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addUserBooking = (movieBooking: MovieDetails) => {
    const existingMovieBooking = userBooking.find((element) =>
      element.title === movieBooking.title,
    );
    if(existingMovieBooking){
      setUserBooking((prevUserBooking) =>
      prevUserBooking.map((item) =>
        item.title === movieBooking.title
          ? { ...item, bookedSeats: item.bookedSeats ? item.bookedSeats + movieBooking.bookedSeats : movieBooking.bookedSeats }
          : item
      ));
    }else{
...
export const useUserBookings = () => {
    ...
    if (!context) {
        throw new Error('retrieveUserBookings must be used within an UserBookingContextProvider');
    }
    return context;
};
